...
export class MapleCube {
  ...
  constructor(
    public options: { level: Level; weapon: Weapon; attackType: AttackType },
  ...
  private async findItems() {
    ...
    for (let y = this.currentPosY; y < this.itemSearchY; ++y) {
      for (let x = this.currentPosX; x < this.itemSearchX; ++x) {
        ...
        if (currentItem != null) {
          this.currentItemPos = new Region(
            currentItem.left + 2,
            currentItem.top + 2,
            currentItem.width - 8,
  ...
  private async parseCubeText() {
    ...
    const grade = emptyStringReplace(
      await getText(new Region(left + 50, top + 144, 75, 14), config),
    );

    const first = await this.parseOption(
      new Region(left + 2, top + 160, 173, 14),
    );
    const second = await this.parseOption(
      new Region(left + 2, top + 174, 173, 14),
    );
    const third = await this.parseOption(
      new Region(left + 2, top + 188, 173, 14),
    );

    const result = [first, second, third].reduce<Result>(
      (accu, curr) => {
        if (curr == null) {
          return accu;
        }
  ...
  private async parseOption(searchRegion: Region): Promise<Option | null> {
    ...
    for await (const path of paths) {
      const region = await getImage(`src/assets/maple/cube/${path}.png`, {
        searchRegion,
      });
  ...
  public async openCube() {
    ...
    if (region == null) {
      throw new Error('수상한 큐브가 존재하지 않습니다.');
    }
  ...
  private async retry() {
    if (this.needRetrySearch) {
      ...
      if (retryRegion == null) {
        throw new Error('한번 더 사용하기 버튼을 찾을 수 없습니다.');
      }
  ...
  public async release() {
    ...
    while (true) {
      ...
      if (isValid) {
        ...
        const confirmRegion = await getImage(
          `${this.resourcePath}/confirm2.png`,
        );
