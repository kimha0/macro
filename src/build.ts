...
async function main() {
  ...
  const trailoring = new Tailoring(Key.Num2, 350, { isPerfectFinish: false, usesLeftCount: 11 });
  ...
  await asyncTask
    .build(() => trailoring.update(), () => trailoring.createCount < (50 * loopCount))
    .build(() => logger(`${trailoring.createCount}번 만들고 분해 시작함.`))
    .build(() => dissolution.update(), () => dissolution.count < trailoring.createCount)
    .sleep(2000)
